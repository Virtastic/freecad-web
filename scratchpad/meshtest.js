...
const CODE=fs.readFileSync('/tmp/meshtest.py','utf8');
...
(async()=>{
 let perr=0;
 const b=await puppeteer.launch({executablePath:CHROME,headless:false,defaultViewport:null,args:['--no-sandbox','--use-gl=angle','--use-angle=metal','--enable-features=SharedArrayBuffer','--window-size=1400,900'],protocolTimeout:1200000,userDataDir:'/tmp/fc-meshtest'});
 ...
 await runG(p,CODE);
 const done=await pw(p,'MT-END',300000); await sl(2000);
 const log=await p.evaluate(()=>document.getElementById('log').textContent);
 const out=log.split('\n').filter(x=>/MT-/.test(x)).join('\n')+'\n reachedEnd='+done+' pageErrs='+perr;
 fs.writeFileSync('/tmp/meshtest.txt',out); console.log(out);
 await b.close().catch(()=>{}); process.exit(0);
})().catch(e=>{fs.writeFileSync('/tmp/meshtest.txt','DRIVER-ERR '+String(e).slice(0,250));process.exit(0);});
