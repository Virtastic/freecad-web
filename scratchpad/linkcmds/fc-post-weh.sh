...
set -e
. toolchain/env.sh
ROOT="$PWD"; CPY="$ROOT/deps/src/cpython"; INST="$ROOT/freecad-gui-install"; PYMT="$ROOT/deps/src/cpython"
cp build-freecad-gui-weh/bin/FreeCAD.js   play-gui/
cp build-freecad-gui-weh/bin/FreeCAD.wasm play-gui/
cp build-freecad-gui-weh/bin/FreeCAD.data play-gui/ 2>/dev/null || true
...
a0_old='''  if (HEAPU32[((0) >>> 2) >>> 0] != 1668509029) /* 'emsc' */ {
    abort("Runtime error: The application has corrupted its heap memory area (address zero)!");
  }'''
a0_new='''  if (HEAPU32[((0) >>> 2) >>> 0] != 1668509029) /* 'emsc' */ {
...
mat_new="""  } else if (pname == 5632) {
    ...
    GLEmulation.materialEmission[0] = HEAPF32[((param) >>> 2) >>> 0];
    GLEmulation.materialEmission[1] = HEAPF32[(((param) + (4)) >>> 2) >>> 0];
    GLEmulation.materialEmission[2] = HEAPF32[(((param) + (8)) >>> 2) >>> 0];
    GLEmulation.materialEmission[3] = HEAPF32[(((param) + (12)) >>> 2) >>> 0];
  } else {
...
lm_old="""  if (pname == 2899) {
    ...
    GLEmulation.lightModelAmbient[0] = HEAPF32[((param) >>> 2) >>> 0];
    GLEmulation.lightModelAmbient[1] = HEAPF32[(((param) + (4)) >>> 2) >>> 0];
    GLEmulation.lightModelAmbient[2] = HEAPF32[(((param) + (8)) >>> 2) >>> 0];
    GLEmulation.lightModelAmbient[3] = HEAPF32[(((param) + (12)) >>> 2) >>> 0];
  } else {
    throw \"glLightModelfv: TODO: \" + pname;
  }"""
lm_new="""  if (pname == 2899) {
    var FCWEB_AMB = 0.55;
    GLEmulation.lightModelAmbient[0] = Math.max(FCWEB_AMB, HEAPF32[((param) >>> 2) >>> 0]);
    GLEmulation.lightModelAmbient[1] = Math.max(FCWEB_AMB, HEAPF32[(((param) + (4)) >>> 2) >>> 0]);
    GLEmulation.lightModelAmbient[2] = Math.max(FCWEB_AMB, HEAPF32[(((param) + (8)) >>> 2) >>> 0]);
    GLEmulation.lightModelAmbient[3] = HEAPF32[(((param) + (12)) >>> 2) >>> 0];
  } else {
  }"""
if lm_old in s:
...
_m=_re.search(r"else if\(pname==5633\)\{GLEmulation\.materialShininess\[0\]=(HEAPF32|GROWABLE_HEAP_F32\(\))\[param(>>>2>>>0|>>2)\]\}else\{0\}\}var _emscripten_glMaterialfv", s)
if 'pname==5634' in s: out.append('mat-color:already')
elif _m:
    H=_m.group(1); SH=_m.group(2)
    def _hf(off):
        return (H+'[param'+('+'+str(off) if off else '')+SH+']') if not off else (H+'[param+'+str(off)+SH+']')
    mad_new=('else if(pname==5633){GLEmulation.materialShininess[0]='+H+'[param'+SH+']}'
     'else if(pname==5632){GLEmulation.materialEmission[0]='+H+'[param'+SH+'];'
     'GLEmulation.materialEmission[1]='+H+'[param+4'+SH+'];'
     'GLEmulation.materialEmission[2]='+H+'[param+8'+SH+'];'
     'GLEmulation.materialEmission[3]='+H+'[param+12'+SH+']}'
    ...
    s=s.replace(_m.group(0),mad_new,1); out.append('mat-color:OK')
...
# forces a synchronous GPU round-trip under ANGLE/Metal (~ms each) -> during real mouse
# interaction (thousands of small immediate-mode batches per redraw) this dominated the
# frame at ~1 FPS / 36s of a 40-move profile. isProgram only ever changes answer when the
# GL context is recreated (Qt-wasm does this on resize/window events), so cache the result
# per-renderer tagged with the current GLctx and re-validate only when the context swaps.
