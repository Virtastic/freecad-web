...
(async () => {
  const b = await puppeteer.launch({
    ...
    protocolTimeout: 2400000, userDataDir: '/tmp/fc-probe-rel',
  });
  ...
  await p.goto('http://localhost:8797/index.html', { waitUntil: 'domcontentloaded', timeout: 300000 });
  const t0 = Date.now();
  ...
  fs.writeFileSync('/tmp/probe-rel.txt', out);
  console.log(out);
  await b.close().catch(() => {});
  process.exit(0);
})().catch((e) => {
  fs.writeFileSync('/tmp/probe-rel.txt', 'DRIVER ' + String(e).slice(0, 300));
  process.exit(0);
});
