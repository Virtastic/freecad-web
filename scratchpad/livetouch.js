...
(async()=>{
 ...
 const b=await puppeteer.launch({executablePath:CHROME,headless:false,defaultViewport:null,
   ...
   protocolTimeout:900000,userDataDir:'/tmp/fc-livetouch'});
 ...
 await p.goto('https://freecad.virtastic.app/',{waitUntil:'domcontentloaded',timeout:240000});
 ...
 await p.screenshot({path:'/tmp/live-tablet.png'});
 ...
 fs.writeFileSync('/tmp/livetouch.txt',R.join('\n')); console.log(R.join('\n'));
 await b.close().catch(()=>{}); process.exit(0);
})().catch(e=>{fs.writeFileSync('/tmp/livetouch.txt','DRIVER-ERR '+String(e).slice(0,250));process.exit(0);});
