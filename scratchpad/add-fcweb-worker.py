"""One persistent worker thread for the heavy calls a document load makes off the main
thread (BRep parse, tessellation), as a header-only helper src/Base/FcwebWorker.h added to
patches/freecad.patch as a new-file section. Spawning a std::thread per object and joining
it on the main thread cost ~50 ms an object (BIMExample open 7.4 -> 26.1 s, 2026-09-16);
posting to one long-lived thread and parking on its done flag costs a wake."""
import io, os

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
P = os.path.join(ROOT, 'patches', 'freecad.patch')
REL = 'src/Base/FcwebWorker.h'
...
if lines[-1] == '':
    lines = lines[:-1]
body = ''.join('+' + l + '\n' for l in lines)
sec = ('diff -ruNp a/%s b/%s\n--- a/%s\n+++ b/%s\n@@ -0,0 +1,%d @@\n' % (REL, REL, REL, REL, len(lines))).encode() + body.encode()
raw = io.open(P, 'rb').read()
hdr = ('diff -ruNp a/%s ' % REL).encode()
if hdr in raw:
    i = raw.index(hdr); j = raw.index(b'\ndiff -ruNp ', i + 10) + 1
    raw = raw[:i] + sec + raw[j:]
else:
    anchor = b'diff -ruNp a/src/Base/'
    i = raw.index(anchor) if anchor in raw else raw.index(b'diff -ruNp a/src/Gui/')
    raw = raw[:i] + sec + raw[i:]
io.open(P, 'wb').write(raw)
io.open(os.path.join(ROOT, 'scratchpad', 'FcwebWorker.h'), 'w', newline='\n').write(HEADER)
print('section: %d lines' % len(lines))
