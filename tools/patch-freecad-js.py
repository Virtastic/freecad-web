...
_DLISTS = (
    ...
    'for(var k=0;k<names.length;k++){var ai=names[k][1];if(!cl.en[ai])continue;var ca=cl.p[ai];if(!ca||!ca.pointer)continue;var ts=GL.byteSizeByType[ca.type-GL.byteSizeByTypeRoot]||4,es=ca.size*ts,st=ca.stride||es,bytes=(vcount-1)*st+es;var np2=__fcDL.keep(R,ca.pointer,bytes);if(!np2)return null;'
