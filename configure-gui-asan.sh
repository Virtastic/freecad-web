...
emcmake cmake -S deps/src/freecad -B build-freecad-gui-asan -G Ninja \
  -DCMAKE_POLICY_VERSION_MINIMUM=3.5 \
  -DCMAKE_PROJECT_INCLUDE_BEFORE="$ROOT/force-static.cmake" \
  -DCMAKE_INSTALL_PREFIX="$ROOT/freecad-gui-asan-install" \
  -DFCWEB_DW="$DW" \
  ...
  -DCMAKE_CXX_FLAGS="-fexceptions -pthread -O1 -g -fsanitize=address -DBOOST_ALL_NO_LIB --use-port=zlib -include $DW/include/gl_compat.h -include $DW/include/coin_intrusive.h" \
  -DCMAKE_C_FLAGS="-fexceptions -pthread -O1 -g -fsanitize=address --use-port=zlib -include $DW/include/gl_compat.h -include $DW/include/coin_intrusive.h" \
  -DCMAKE_EXE_LINKER_FLAGS="$FC_LINK_MODE_FLAGS -O1 -g -fsanitize=address -sINITIAL_MEMORY=2147483648 -lembind -pthread -sALLOW_MEMORY_GROWTH -sPTHREAD_POOL_SIZE=4 -sASSERTIONS=1 -sFORCE_FILESYSTEM=1 -sMODULARIZE=1 -sEXPORT_NAME=FreeCAD_entry -sWASM_BIGINT=1 -sSTACK_SIZE=32MB -sDEFAULT_PTHREAD_STACK_SIZE=16MB -sMAXIMUM_MEMORY=4GB -sINITIAL_MEMORY=1073741824 -sMAX_WEBGL_VERSION=2 -sLEGACY_GL_EMULATION=1 -sGL_UNSAFE_OPTS=0 -sERROR_ON_UNDEFINED_SYMBOLS=0 -g2 -sFETCH -sEXPORTED_RUNTIME_METHODS=UTF16ToString,stringToUTF16,UTF8ToString,stringToUTF8,JSEvents,specialHTMLTargets,FS,ENV,callMain -sEXPORTED_FUNCTIONS=_main,__embind_initialize_bindings -Wl,--allow-multiple-definition $ROOT/gl_legacy_stubs.o --pre-js=$ROOT/pre-gui.js --use-port=zlib --use-port=bzip2 --use-port=sqlite3 $PYMT/Modules/_decimal/libmpdec/libmpdec.a $PYMT/Modules/_hacl/libHacl_Hash_SHA2.a $PYMT/Modules/expat/libexpat.a"
