...
| **R2** | `QEventLoop::exit` null function | **still not reproducible, and the test is now worth more than it was.** It was reported on a real boot with 3D on, and every attempt to reproduce it ran the restore path with a 3D pipeline that -- as the pixel work later showed -- was reading back a black canvas. So "3D on" had not really been tested. Re-run against the current artifact with the GL path demonstrably drawing (196 colours, 27.9% of the frame): restore still passes, `RestoreProbe` comes back with volume 480.0, and no null function appears. The detector stays in the gate's fatal list so a recurrence fails CI rather than being noticed by a person. |
...
| **V7** | every link costs ~110 minutes | **FIXED, measured across five runs.** Compile 88 min -> 22 min (95.6% ccache hits, reproduced on two consecutive warm runs; two cold baselines at 87.9 and 88.5; the populate run cost nothing measurable). Total per link ~110 -> ~45. Measured on run 33004112792: Compile 87.9 min (79.8%), Link 15.3, rest ~7 -- 110.1 total, and the compile ended `reached: [1860/1860]`, every target rebuilt despite a 421 MB build-tree cache restoring. The mechanism is more specific than "the tree is re-patched": a tag tarball carries the TAG's mtimes and tar restores them, so the ~2,700 pristine sources come back OLDER than the cached objects and are correctly up to date. `patches/apply.sh` then rewrites ~200 files with patch(1), stamping them NOW, and those 200 fan out to 1,860 edges. So 1,860 is a deterministic floor, not bad luck -- 22.7 CPU-seconds per translation unit, spent reproducing byte-identical objects. Fixed by ccache (`EM_COMPILER_WRAPPER`, content-addressed, `compiler_check=content`, sloppiness empty) rather than by caching the patched tree, which is the shape that once cost this project its boot. Also NOT chosen: touching the patched files back to a fixed mtime -- faster, and wrong in the "should rebuild but doesn't" direction, which is silent. ccache errs the other way: an unrecognised input is a miss, and a miss is slow, not wrong. The remaining 4.4%% of misses are the `__DATE__`/`__TIME__` translation units, which ccache refuses by default and which `sloppiness` is deliberately left empty to keep refusing. Verified rather than trusted: a post-compile gate re-runs ~15 objects under `CCACHE_DISABLE=1` and compares sha256, which also proves these wasm objects are bit-reproducible. |
...
| V6 | Does the 3D view render correctly on real hardware? | **Measurable now, and the first measurement says the viewport renders a shaded solid.** The old premise -- that a headless run cannot judge rendering -- was wrong twice: rAF fires (179 ticks in 3 s), SwiftShader gives real WebGL2, and `?gltrace=1` shows **399 draw calls** for one box. Every capture read black because Coin renders into its own offscreen FBO and the C++ blits that into Qt's backing FBO, so the canvas default framebuffer is black **by construction**. `?pixelgate=1` (new) keeps the drawing buffer and records each framebuffer OBJECT as it is bound -- an id cannot be resolved to an object because emscripten's GL map is not exported. Reading those: **146,322 of 520,000 pixels non-background, 195 distinct colours** -- 176,184,191 (FreeCAD's default shape colour) on 70,103 px, 77,84,89 on 52,615, black edges on 7,365. A shaded solid with outlines. What this does NOT yet prove is correctness against desktop: it is SwiftShader, and nobody has compared it to a reference image. That comparison is now a small job rather than an impossible one. |
