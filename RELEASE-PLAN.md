...
| **R11** | the shipped FEM example crashes the engine | **fixed, and the fix verified in the built libraries.** Opening `FEMExample.FCStd` trapped with `RuntimeError: unreachable` in `vtkXMLParser::GetXMLByteIndex` -- an expat `XML_Index` width mismatch that wasm-ld resolves to a trapping stub. `patches/vtk-expat-wasm-xmlsize.patch` prevents exactly that and was applied by nothing; `apply.sh` applies it now and VTK was rebuilt (824 s). Checked in the artifacts the link will consume, not in the patch: the generated `include/vtk-9.3/vtk_expat.h` reads `/* #undef XML_LARGE_SIZE */`, and neither `libvtkexpat-9.3.a` (the provider) nor `libvtkIOXMLParser-9.3.a` (the consumer) carries `XML_LARGE_SIZE` -- both halves are 32-bit and agree. The gate's `examples` scenario opens all seven and is what will confirm it end to end. |
...
| V6 | Does the 3D view render correctly on real hardware? | ROADMAP Tier 2 #4: nine fixed-function GL calls silently do nothing. Establish whether any affects correct output, as opposed to performance. **Half-answered: the premise that a headless run cannot judge this is wrong.** `tools/pixel-probe.py` measures it -- headless Chromium on SwiftShader gives a real WebGL2 context and rAF ticks 179 times in 3 s. Three things then have to be got right to see the pixels at all (Qt's canvas is in a shadow root, the near-black loading overlay hides it, and a WebGL canvas must be copied inside a rAF or it reads back cleared). With all three handled the canvas is still uniformly black while Coin reports traversing the scene, so the question is now the specific one -- why the legacy GL emulation draws nothing under SwiftShader -- rather than "we need a GPU in CI". |
