...
## Relinking FreeCAD: four things the link does NOT do for you
...
3. **The stdlib preload is the SOURCE tree, and one module only exists in the build
   tree.** `--preload-file deps/src/cpython/Lib@/pylib` ships CPython's `Lib/`, but
   `_sysconfigdata__emscripten_wasm64-emscripten.py` is generated by the build into
   `builddir/emscripten-mt/build/lib.emscripten-wasm64-3.13/`, so the image never had it
   and `sysconfig.get_config_vars()` raised `ModuleNotFoundError` -- which took `import
   zoneinfo` down (its `_tzpath` asks sysconfig for `TZPATH`), and with it tzlocal and the
   Lens add-on's `InitGui.py` (measured 2026-09-22). Shipped without a relink: the page
   writes `play-gui/am/_sysconfigdata__emscripten_wasm64-emscripten.py` (that build
   output, build root rewritten to `/build`) into the interpreter's site-packages in
   `preRun`, next to `sitecustomize.py`. A future link can preload the file from the
   build tree instead; the page copy is harmless alongside it (same name, same content).
